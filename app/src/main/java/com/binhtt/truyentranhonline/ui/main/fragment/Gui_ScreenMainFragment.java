...
import com.binhtt.truyentranhonline.ui.main.activities.Gui_Screen2Activity_;
...
import static com.binhtt.truyentranhonline.ui.main.InitParams.URL_HOME_PAGE;
 ...
 * @since 16/08/2017
 */
public class Gui_ScreenMainFragment extends BaseFragment {
    ...
    private void getListData() {
        if (ConnectionUtil.isConnected(getContext())) {
            new MainHomeTask().execute(URL_HOME_PAGE);
        } else {
    ...
    private class MainHomeTask extends AsyncTask<String, Void, ArrayList<Story>> {
        @Override
        ...
        protected void onPostExecute(final ArrayList<Story> stories) {
            ...
            mAdapter = new Gui_Screen2Adapter(getContext(), stories, new Gui_Screen2Adapter.OnClickStoryListener() {
                ...
                public void onItemClick(Story story, int position) {
                    if (stories.get(8) == story) {
                        Gui_Screen2Activity_.intent(getActivity()).mStory(story).imageOrLink(true).start();
                    } else {
                        Gui_Screen2Activity_.intent(getActivity()).mStory(story).imageOrLink(false).start();
                    }
                }
            });
